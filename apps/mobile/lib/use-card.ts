  ...
  return useQuery<CardDetail | null>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('cards')
        .select(
          'id, name, image_url, rarity, card_number, popularity_score, heating_up_score, current_price, ebay_avg_price, tcgplayer_market_price, last_price_check_at, brand_id, category, is_sealed, set_id, external_ids, sets(name, release_date), brands(name)',
        )
        .eq('id', id)
        .single();
